...
const [pricingMenu] = PublicKey.findProgramAddressSync([Buffer.from('sap_pricing'), agent.toBuffer()], PROGRAM_ID);
const [globalRegistry] = PublicKey.findProgramAddressSync([Buffer.from('sap_global')], PROGRAM_ID);

const accounts = process.env.INCLUDE_PRICING_MENU === '0'
  ? { wallet: kp.publicKey, agent, agentStats: stats, globalRegistry, systemProgram: SystemProgram.programId }
  : { wallet: kp.publicKey, agent, agentStats: stats, pricingMenu, globalRegistry, systemProgram: SystemProgram.programId };
...
const infos = await conn.getMultipleAccountsInfo([agent, stats, pricingMenu, globalRegistry], 'confirmed');
console.log(JSON.stringify({
  wallet: kp.publicKey.toBase58(),
  agent: agent.toBase58(), stats: stats.toBase58(), pricingMenu: pricingMenu.toBase58(), globalRegistry: globalRegistry.toBase58(),
  estimatedFeeLamports: fee,
  existing: { agent: !!infos[0], stats: !!infos[1], pricingMenu: !!infos[2], globalRegistry: !!infos[3] },
